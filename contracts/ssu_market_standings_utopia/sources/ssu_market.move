...
/// with standings-based authorization via market_standings.
///
/// SsuConfig links an SSU to a Market<T> (from market_standings). Trade functions
/// extract tribe_id and char_id from &Character and pass them to market_standings
/// for standings checks.
///
...
module ssu_market_standings::ssu_market;

use sui::coin::{Self, Coin};
use sui::event;
use sui::clock::Clock;
use market_standings::market_standings::{Self, Market};
use standings_registry::standings_registry::StandingsRegistry;
...
/// Market with standings check (min_trade). Authorized users only.
/// Extracts tribe_id and char_id from the seller's Character.
public fun escrow_and_list<T>(
    ...
    // Post listing on the Market (checks min_trade standing)
    market_standings::post_sell_listing<T>(
        market, registry, tribe_id, char_id,
        config.ssu_id, type_id, price_per_unit, qty, clock, ctx,
    );
}
...
public fun cancel_listing<T>(
    ...
    let listing = market_standings::borrow_sell_listing(market, listing_id);
    assert!(market_standings::listing_seller(listing) == ctx.sender(), ENotListingSeller);
    let type_id = market_standings::listing_type_id(listing);
    let quantity = market_standings::listing_quantity(listing);

    // Remove listing from market
    market_standings::remove_sell_listing<T>(market, listing_id);
...
/// with standings check (min_trade). No owner/delegate authorization required.
/// Extracts tribe_id and char_id from the seller's Character.
...
/// items -> buyer. Checks min_buy standing via the buyer's Character.
/// Returns change coin.
...
public fun buy_from_listing<T>(
    ...
    // Check buyer's standing against min_buy threshold
    let buyer_tribe_id = buyer_character.tribe();
    let buyer_char_id = in_game_id::item_id(&buyer_character.key());
    market_standings::check_standing(
        market, registry, buyer_tribe_id, buyer_char_id,
        market_standings::market_min_buy(market),
    );

    // Read listing data
    let listing = market_standings::borrow_sell_listing(market, listing_id);
    let price_per_unit = market_standings::listing_price_per_unit(listing);
    let available = market_standings::listing_quantity(listing);
    let seller = market_standings::listing_seller(listing);
    let type_id = market_standings::listing_type_id(listing);
    ...
    // Calculate fee
    let fee_bps = market_standings::market_fee_bps(market);
    let fee_recipient = market_standings::market_fee_recipient(market);
    ...
    if (remaining == 0) {
        ...
        let listing_mut = market_standings::borrow_sell_listing_mut(market, listing_id);
        market_standings::set_listing_quantity(listing_mut, remaining);
    };
...
/// Escrowed payment released to seller (minus fee). No standings check --
/// the buyer already passed min_buy when posting the order.
#[allow(lint(self_transfer))]
public fun player_fill_buy_order<T>(
    ...
    // Read buy order data
    let order = market_standings::borrow_buy_order(market, order_id);
    let price_per_unit = market_standings::order_price_per_unit(order);
    let available = market_standings::order_quantity(order);
    let buyer = market_standings::order_buyer(order);
    assert!(type_id == market_standings::order_type_id(order), ETypeMismatch);
    assert!((quantity as u64) <= available, EInsufficientQuantity);
    ...
    if (fee_amount > 0) {
        let fee_coin = market_standings::split_escrowed_coin<T>(market, order_id, fee_amount, ctx);
        transfer::public_transfer(fee_coin, fee_recipient);
    };
    let seller_coin = market_standings::split_escrowed_coin<T>(market, order_id, seller_amount, ctx);
    ...
    let remaining = available - (quantity as u64);
    if (remaining == 0) {
        market_standings::remove_buy_order<T>(market, order_id);
        let remaining_coins = market_standings::remove_escrowed_coin<T>(market, order_id);
        ...
        let order_mut = market_standings::borrow_buy_order_mut(market, order_id);
        market_standings::set_order_quantity(order_mut, remaining);
    };
...
public fun fill_buy_order<T>(
    ...
    let price_per_unit = market_standings::order_price_per_unit(order);
    let available = market_standings::order_quantity(order);
    let type_id = market_standings::order_type_id(order);
    let buyer = market_standings::order_buyer(order);
    assert!((quantity as u64) <= available, EInsufficientQuantity);
