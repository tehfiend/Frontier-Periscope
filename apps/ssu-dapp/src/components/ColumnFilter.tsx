...
import { ArrowDown, ArrowUp, ChevronDown, ChevronRight, ListFilter, X } from "lucide-react";
...
export function ColumnFilter<TData>({ column, valueLabels }: ColumnFilterProps<TData>) {
	...
	const [textFilterType, setTextFilterType] =
		useState<ExcelFilterValue["textFilterType"]>("contains");
	...
	const included =
		pendingIncluded ??
		(currentFilter?.mode === "include" ? currentFilter.includedValues : null) ??
		new Set(allValues);
		...
		<div>
			<button
				...
				className={`rounded p-0.5 transition-colors ${
					isFiltered ? "text-cyan-400 hover:text-cyan-300" : "text-zinc-600 hover:text-zinc-400"
				}`}
				title="Filter column"
			>
				<ListFilter size={12} />
								...
								{textFilterOpen && (
									<div className="space-y-2 px-3 pb-3">
										<select
											...
											onChange={(e) =>
												setTextFilterType(e.target.value as ExcelFilterValue["textFilterType"])
											}
