...
function PeriscopeIcon({ size = 16, className = "" }: { size?: number; className?: string }) {
	return (
		<svg
			width={size}
			height={size}
		...
		>
			...
			<path d="m787.5 581.26h-375c-124.08 0-225 100.92-225 225 0 124.08 100.92 225 225 225 66.797 0 129.74-29.578 172.69-81.188 7.5938-9.1406 21.984-9.1406 29.578 0 42.938 51.609 105.89 81.188 172.69 81.188 124.08 0 225-100.92 225-225 0-124.08-100.92-225-225-225zm-375 375c-82.688 0-150-67.312-150-150s67.312-150 150-150 150 67.312 150 150-67.312 150-150 150zm375 0c-40.078 0-77.766-15.609-106.08-43.922s-43.922-66-43.922-106.08c0-82.688 67.312-150 150-150 40.266 0 76.359 15.094 104.39 43.641 29.625 27.891 45.609 64.828 45.609 106.36 0 82.688-67.312 150-150 150z" />
						...
						info.status === "default"
							...
							: info.status === "periscope"
								...
								: info.status === "periscope-outdated"
									? "Update"
									: "Custom";
					if (extConfig?.registryName) return `${label} (${extConfig.registryName})`;
					return label;
				},
				header: "Extension",
				size: 140,
								...
								<>
									...
									{r.ownership === "mine" && (
										<button
											type="button"
											onClick={() => setDeployTarget(r)}
											className="ml-auto rounded px-1.5 py-0.5 text-[10px] font-medium text-amber-400 hover:bg-amber-900/30"
										>
											Update
										</button>
									)}
