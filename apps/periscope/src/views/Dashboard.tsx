...
const quickActions: { to: string; icon: LucideIcon; label: string; chain?: boolean }[] = [
		...
		<div className="flex h-full flex-col overflow-y-auto">
			<div className="mx-auto w-full max-w-4xl px-6 py-8">
				...
				<div className="mb-6 flex flex-wrap gap-2">
					{quickActions
						.filter((a) => CHAIN_ENABLED || !a.chain)
						.map((action) => (
							<Link
								key={action.to}
								to={action.to}
				...
				<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
					...
					{CHAIN_ENABLED && (
						<DashboardCard icon={Lock} title="Private Maps" to="/private-maps">
							{totalMaps > 0 ? (
								...
								<p>
									Private maps store encrypted structure locations that only invited members can
									see. Use them to share intel with allies without revealing positions publicly.
								</p>
							)}
						</DashboardCard>
					)}
					...
					<DashboardCard icon={BookUser} title="Standings" to="/standings">
						{contactCount > 0 || (CHAIN_ENABLED && registryCount > 0) ? (
							<p>
								<span className="text-zinc-200">
							...
							{ownedDeployables.length > 0 ? (
								<div>
									...
									<div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-xs">
										...
										{fuel.noFuel > 0 && (
											<span className="text-zinc-500">{fuel.noFuel} no data</span>
										)}
									</div>
								</div>
							) : (
