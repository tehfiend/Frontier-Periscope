	...
	useEffect(() => {
		async function load() {
			...
			try {
				...
				// Check for shared ACL binding by scanning dynamic fields
				const binding = await detectSharedAclBinding(client, configObjectId, assemblyId, packageId);
				if (binding) {
					setSharedAclBinding(binding);
					setSelectedSharedAclId(binding.sharedAclId);
					setSharedPermitDurationMin(Math.round(binding.permitDurationMs / 60_000));
					setAclMode("shared");

					// Load the bound ACL's details
				...
				<div className="space-y-4">
						...
						<>
							...
							{sharedAclBinding && (
								<div className="flex items-center gap-2 rounded border border-amber-900/50 bg-amber-950/20 p-2 text-xs text-amber-400">
									<AlertCircle size={14} />
									This gate has a shared ACL bound. The inline config below is separate -- switch to
									"Shared ACL" mode to manage it.
								</div>
							)}
							...
							{tribeIds.length + characterIds.length > 80 && (
								<p className="text-xs text-amber-400">
									Large ACL lists (&gt;100 entries) may exceed gas limits. Consider using
									tribe-level grouping.
								</p>
							)}
					...
					{aclMode === "shared" && (
						<>
									...
									{selectedAclDetails ? (
										<div className="space-y-1">
											...
											<div className="flex gap-3 text-[10px] text-zinc-500">
												...
												>
													{selectedAclDetails.isAllowlist ? "Allowlist" : "Denylist"}
												</span>
												<span>
													{selectedAclDetails.allowedTribes.length} tribe
													{selectedAclDetails.allowedTribes.length !== 1 ? "s" : ""}
												</span>
												<span>
													{selectedAclDetails.allowedCharacters.length} character
													{selectedAclDetails.allowedCharacters.length !== 1 ? "s" : ""}
												</span>
											</div>
											<p className="text-[10px] text-zinc-600">
												Permit duration: {Math.round(sharedAclBinding.permitDurationMs / 60_000)}{" "}
												min
											</p>
										</div>
									) : (
									...
									>
										{isSaving ? (
											<>
												...
												{status === "signing" ? "Waiting for wallet..." : "Removing..."}
											</>
										) : (
											<>
												<Unlink size={14} />
							...
							<div>
								<label className="mb-1 block text-xs text-zinc-500">
									{sharedAclBinding ? "Change Shared ACL" : "Select Shared ACL"}
								</label>
								...
								<input
									...
									onChange={(e) => setSelectedSharedAclId(e.target.value)}
									...
									<div className="mt-2">
										<div className="relative">
											...
											<input
												...
												onChange={(e) => setAclSearchQuery(e.target.value)}
										...
										{!loadingAcls && filteredAcls.length === 0 && (
											<p className="mt-2 text-[10px] text-zinc-600">
												No shared ACLs found. Create one in the "Shared ACLs" tab first.
											</p>
										)}
									</div>
								)}
								...
								{!aclRegistryPackageId && (
									<p className="mt-1.5 text-[10px] text-zinc-600">
										Enter the ACL Registry package ID on the "Shared ACLs" tab to browse available
										ACLs, or paste an object ID directly above.
									</p>
								)}
							</div>

							{/* Selected ACL details preview */}
							{selectedAclDetails && selectedSharedAclId !== sharedAclBinding?.sharedAclId && (
								<div className="rounded border border-zinc-800 bg-zinc-900/30 p-3">
									...
									<div className="space-y-0.5 text-[10px] text-zinc-500">
										...
										{selectedAclDetails.allowedTribes.length > 0 && (
											<p>
												Tribes: {selectedAclDetails.allowedTribes.map((t) => `#${t}`).join(", ")}
											</p>
										)}
										{selectedAclDetails.allowedCharacters.length > 0 && (
											<p>
												Characters:{" "}
												{selectedAclDetails.allowedCharacters.map((c) => `#${c}`).join(", ")}
											</p>
										)}
										<p>
											Creator: {selectedAclDetails.creator.slice(0, 12)}...
							...
							<div>
								...
								<input
									...
									onChange={(e) => setSharedPermitDurationMin(Number(e.target.value))}
									min={1}
									className="w-24 rounded border border-zinc-700 bg-zinc-800 px-2 py-1.5 text-xs text-zinc-100 focus:border-cyan-500 focus:outline-none"
								/>
							</div>
...
/**
 * Query the SharedAclConfig dynamic field for a specific gate by scanning the
 * ExtensionConfig's dynamic fields for a SharedAclKey entry matching the gate ID.
 *
 * SharedAclKey is a struct type `{packageId}::config::SharedAclKey` with a
 * `gate_id: ID` field. We detect it by checking the dynamic field key type
 * and matching the gate_id value.
 *
 * Returns the binding info (sharedAclId + permitDurationMs) if found, null otherwise.
 */
async function detectSharedAclBinding(
	...
	// Use a dynamicFields scan with type filtering to find the SharedAclConfig
	const QUERY = `
	...
	try {
		...
		for (const node of nodes) {
			...
			// Extract the value -- could be MoveValue or MoveObject
