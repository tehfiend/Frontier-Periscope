...
4. **TODO** -- Update `apps/permissions-dapp/src/components/AclEditor.tsx`:
   - Add "Use Shared ACL" toggle/section below the existing inline ACL form
   - When enabled, show a search/dropdown to select a SharedAcl by name or ID
   - Show permit duration input
   - Save calls `buildSetSharedAclConfig` instead of `buildConfigureAcl`
   - Display indicator when gate is currently using a shared ACL (query `has_shared_config`)
   - **Note:** The chain-shared builders `buildSetSharedAclConfig` and `buildRemoveSharedAclConfig` already exist in `permissions.ts`. Only the UI integration is missing.
...
| `apps/permissions-dapp/src/components/AclEditor.tsx` | MODIFY | **TODO** | Add "Use Shared ACL" section to bind gates to SharedAcl |
...
1. **AclEditor shared ACL binding (Phase 2, Step 4):** The `AclEditor.tsx` component in `permissions-dapp` does not yet have a "Use Shared ACL" toggle. The chain-shared builders (`buildSetSharedAclConfig`, `buildRemoveSharedAclConfig`) are already implemented -- only the AclEditor UI integration is missing. This is the only functional gap: without it, gate owners cannot bind their gates to shared ACLs from the dApp UI.

2. **Constants.ts updates (Phase 4, Step 6):** The `ssu-market-dapp` constants.ts does not have `CURRENCY_MARKET_PACKAGE_ID` or `getMarketId()`. This is blocked on contract deployment (package IDs are placeholder zeros). Users can currently enter the package ID manually via an input field, so this is a convenience improvement rather than a blocker.

3. **Contract deployment:** Both `acl_registry` and `currency_market` contracts are written but not yet published to Sui testnet. Package IDs in `config.ts` are placeholder zeros for both tenants. The `gate_acl` upgrade also depends on `acl_registry` being published first.
