...
**Dependency:** This plan requires Plan 04 (Manifest Expansion) to be implemented first, since the dashboard reads market and registry counts from `db.manifestMarkets` and `db.manifestRegistries` tables that Plan 04 creates.
...
- Markets (Plan 04): `db.manifestMarkets.toArray()` -- global cache of all Market<T> objects
- Registries (Plan 04): `db.manifestRegistries.toArray()` -- global cache of all StandingsRegistry objects
- Currencies/markets (existing): `db.currencies.filter(notDeleted).toArray()` (Market.tsx L80)
...
- Quick actions row with navigation links to key module views (Open Sonar, View Structures, etc.)
...
A horizontal row of navigation links below the header, providing one-click access to the most-used views. Each link is a simple text link (with icon) styled as a compact pill/chip. Links:
- Open Sonar -> `/sonar`
...
3. **Standings** -- shows contact count + registry count (from `db.manifestRegistries`). When empty: explains that standings control who can use your structures (SSU access, gate tolls, turret targeting), links to Standings view.
4. **Markets** -- shows count of cached markets (from `db.manifestMarkets`). When empty: explains that markets let you create and trade governance tokens, links to Markets view.
...
| Data queries | Direct `useLiveQuery` calls in Dashboard | Simple, reactive, consistent with other views. No new hooks needed -- reuse `useActiveCharacter`, `useContacts`, `useSubscribedRegistries`. Market/registry counts come from Plan 04 manifest tables. |
| Route change | Replace redirect with component | Change indexRoute from `beforeLoad: redirect` to `component: Dashboard`. |
| Sidebar link label | "Dashboard" with `LayoutDashboard` icon | Placed at top before nav groups. Clear naming that reflects the view's purpose. |
| Sidebar link matching | Exact match (`activeOptions: { exact: true }`) | The `/` path is a prefix of every route, so prefix matching would highlight the Dashboard link on every page. Exact matching ensures it only highlights when actually on the dashboard. Add an optional `exact` prop to `NavLink` to support this. |
| Quick actions | Navigation links row below header | Simple link pills to key module views (Sonar, Structures, Star Map, etc.). Just navigation -- no action triggers. Provides quick access for returning users without cluttering the cards. |
| Market/registry data source | Plan 04 manifest tables (`db.manifestMarkets`, `db.manifestRegistries`) | Accurate counts of all on-chain markets and registries, not just user-interacted ones. This creates a hard dependency on Plan 04. |
...
1. Create `apps/periscope/src/views/Dashboard.tsx` with the Dashboard component:
   ...
   - Queries (all reactive via `useLiveQuery` or existing hooks):
     ...
     - Markets: `db.manifestMarkets.count()` (global -- no tenant filter, per Plan 04 design)
     - Registries: `db.manifestRegistries.count()` (global -- no tenant filter, per Plan 04 design)
   ...
   - Render quick actions row (see step 3a below)
...
3. Quick actions row and card content:

   **3a. Quick actions row** -- rendered between the page header and the module cards grid:
   - Container: `flex flex-wrap gap-2 mb-6`
   - Each action: `<Link to={route} className="...">` styled as a compact pill with icon + label
   - Pill style: `inline-flex items-center gap-1.5 rounded-full border border-zinc-700 bg-zinc-800/50 px-3 py-1.5 text-xs text-zinc-300 hover:border-cyan-500/50 hover:text-cyan-400 transition-colors`
   ...
   **3b. Card content** (use same icons as Sidebar for consistency):
   ...
   - **Standings card** (icon: `BookUser`, to: `/standings`): show `{contactCount} contacts, {registryCount} registries` (registryCount from `db.manifestRegistries.count()`). Empty: "Standings control who can interact with your structures -- gate access, SSU deposits, turret targeting. Create a registry to define friend/foe rules, or add contacts for private tracking."
   - **Markets card** (icon: `Coins`, to: `/markets`): show `{marketCount} markets` (from `db.manifestMarkets.count()`). Empty: "Governance markets let you publish custom tokens and manage buy/sell orders. Create a token to power your organization's economy."
...
5. Update `apps/periscope/src/components/Sidebar.tsx`:
   ...
   - Add an optional `exact` prop to the `NavLink` component's props and the `NavItem` type. When `exact` is truthy, use `activeOptions={{ exact: true }}` on the `<Link>`; otherwise keep the existing `activeOptions={{ exact: false }}`. This is the minimal change -- one ternary on the `activeOptions` prop.
   - Add the Dashboard link in the `<nav>` element (L181) before the `navGroups.map(...)` loop: `<NavLink to="/" icon={LayoutDashboard} label="Dashboard" exact />`
   - Add `mb-4` bottom margin on the Dashboard link's wrapper `<div>` to visually separate it from the first nav group.
...
|------|--------|-------------|
| `apps/periscope/src/views/Dashboard.tsx` | CREATE | New dashboard view with module cards and quick actions row |
| `apps/periscope/src/router.tsx` | MODIFY | Replace index redirect with Dashboard component, add lazy import |
| `apps/periscope/src/components/Sidebar.tsx` | MODIFY | Add "Dashboard" link with LayoutDashboard icon and exact matching |
...
1. **Quick actions section** -- Decided: Module cards + quick actions row. The quick actions row sits between the header and the card grid, containing simple navigation link pills to key module views (Sonar, Structures, Star Map, Private Maps, Standings, Markets). These are pure navigation links, not action triggers.

2. **Sidebar link naming and matching** -- Decided: Label is "Dashboard" (not "Home") with `LayoutDashboard` icon. Uses exact matching (`activeOptions: { exact: true }`) via a new optional `exact` prop on `NavLink`. Exact matching is required because `/` is a prefix of every route -- prefix matching would cause the Dashboard link to appear active on every page. The `NavLink` component gains an `exact?: boolean` prop; when truthy, it sets `activeOptions={{ exact: true }}` instead of the default `{{ exact: false }}`.

3. **Market/registry data source** -- Decided: Use Plan 04 manifest tables. The dashboard reads `db.manifestMarkets.count()` and `db.manifestRegistries.count()` for accurate on-chain totals. This creates a hard dependency: Plan 04 must be implemented before this plan. The existing `db.currencies` table is no longer used for the Markets card.

## Cross-Plan Dependencies

- **Plan 04 (Manifest Expansion) -- HARD DEPENDENCY.** This plan requires Plan 04 to be implemented first. The dashboard reads `db.manifestMarkets` and `db.manifestRegistries` tables that Plan 04 creates. Without Plan 04, the Markets and Standings cards would have no data source for on-chain entity counts.
