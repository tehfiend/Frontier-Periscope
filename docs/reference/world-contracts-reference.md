# EVE Frontier World Contracts Reference (v0.0.18 -- v0.0.23)

Source: https://github.com/evefrontier/world-contracts
Package: `world`, edition `2024.beta`, Sui testnet-v1.66.2

> **Note:** Both tenants are now at version 2 (Utopia upgraded to v0.0.21 on 2026-03-23,
> Stillness upgraded to v0.0.23 on 2026-04-01). On Sui, upgraded packages use the
> **original package ID** for type strings in GraphQL queries, and the **published-at
> address** for `moveCall` targets (to reach new functions like `revoke_extension_authorization`).

## Changelog

### v0.0.23 (Apr 1, 2026)
- Stillness upgraded to parity with Utopia. `Published.toml` updated with Stillness upgraded package IDs (version 2). No Move source changes beyond v0.0.21.
- Toolchain version bumped from 1.67.1 to 1.69.1.

### v0.0.22 (Apr 1, 2026)
- CI/Docker changes only. No Move source changes.
- Docker snapshot image tooling: `docker-compose-snapshot-image.yml`, `entrypoint-snapshot-image.sh`, `bake-snapshot-image.sh`.
- CI workflows updated to publish snapshot images with `-snapshot` tags.

### v0.0.21 (Mar 22, 2026)
- `Published.toml` updated with upgraded package IDs for testnet, testnet_internal, and Utopia (version 2). No Move source changes.

### v0.0.20 (Mar 22, 2026) -- PR #140
- **New function:** `gate::issue_jump_permit_with_id<Auth>` -- same as `issue_jump_permit` but returns `ID` of the minted permit.
- **New event:** `gate::JumpPermitIssuedEvent` -- emitted on all permit issuance (both `issue_jump_permit` and `issue_jump_permit_with_id`). Fields: `jump_permit_id`, `source_gate_id`, `source_gate_key`, `destination_gate_id`, `destination_gate_key`, `character_id`, `character_key`, `route_hash`, `expires_at_timestamp_ms`, `extension_type`.
- Internal refactoring: `issue_jump_permit` now delegates to `issue_jump_permit_internal` which emits the event. New private helpers: `assert_gate_extension_matches<Auth>`, `assert_jump_permit_extension_authorization<Auth>`.

### v0.0.19 (Mar 20, 2026) -- PRs #135, #137
- **Extension revocation:** `revoke_extension_authorization()` added to gate, turret, and storage_unit. Emits `ExtensionRevokedEvent` with fields: `assembly_id`, `assembly_key`, `revoked_extension`, `owner_cap_id`. New error code `ENoExtensionToRevoke` on gate (19), turret (12), storage_unit (16).
- **Turret targeting getters:** `hp_ratio()`, `shield_ratio()`, `armor_ratio()` on `TargetCandidate` -- public getters for health/shield/armor ratios during `get_target_priority_list`.
- **Turret view functions:** `id()`, `key()`, `extension()` (returns `&Option<TypeName>`, non-aborting alternative to `extension_type()`), `metadata()` (promoted from `#[test_only]` to `public`).
- **View ID functions across modules:** `gate::id`, `gate::gate_config_id`, `gate::jump_permit_id`, `gate::route_hash`, `gate::delete_jump_permit`, `gate::delete_jump_permit_with_auth`, `storage_unit::id`, `assembly::id`, `access_control::admin_acl_id`, `access_control::server_address_registry_id`, `character::player_profile_id`, `killmail::id` + all killmail getters (`key`, `killer_id`, `victim_id`, `reported_by_character_id`, `kill_timestamp`, `loss_type`, `solar_system_id`, `is_structure_loss`, `is_ship_loss`), `killmail_registry::id`, `network_node::id`, `energy::id`, `fuel::id`, `inventory::id` (Item), `location::id`, `object_registry::id`, `world::id` (GovernorCap).
- **Metadata getters:** `metadata::name`, `metadata::description`, `metadata::url` promoted from `#[test_only]` to `public`.
- **Visibility change:** `access_control::receive_owner_cap<T>` changed from `public(package)` to `public` -- builders can now directly receive owner caps.
- **Dependencies:** `Published.toml` added to git for downstream dependency resolution.
...
| testnet_stillness | `0x28b497559d65ab320d9da4613bf2498d5946b2c0ae3597ccfda3072ce127448c` | `0xd2fd1224f881e7a705dbc211888af11655c315f2ee0f03fe680fc3176e6e4780` (v0.0.23) | 2 |
...
Adds an **irreversible** freeze marker as a dynamic field on an assembly's UID. Anti-rugpull mechanism -- once frozen, players interacting with an SSU/Gate/Turret can trust the extension rules won't change.

### Structs & Event

```move
public struct ExtensionFrozenKey has copy, drop, store {}   // DF key
public struct ExtensionFrozen has copy, drop, store {}      // DF marker value
...
```move
// Check if frozen (public -- callable by anyone/any package)
public fun is_extension_frozen(object: &UID): bool

// Set frozen marker + emit event (package-private -- only world:: modules)
public(package) fun freeze_extension_config(parent: &mut UID, assembly_id: ID)

// Cleanup on unanchor (package-private)
public(package) fun remove_frozen_marker_if_present(parent: &mut UID)
```

### Per-Assembly Public API

Each assembly (Gate, Turret, StorageUnit) exposes identical functions:

```move
// Freeze (owner-only, one-time, irreversible) -- requires extension IS configured
public fun freeze_extension_config(assembly: &mut T, owner_cap: &OwnerCap<T>)

// View
public fun is_extension_frozen(assembly: &T): bool
```

Preconditions for freeze:
1. Caller holds valid OwnerCap
2. Extension IS configured (`EExtensionNotConfigured` if not)
3. Extension is NOT already frozen (`EExtensionConfigFrozen` if so)

After freeze, `authorize_extension` and `revoke_extension_authorization` both abort with `EExtensionConfigFrozen`.

### Lifecycle

1. Assembly anchored with `extension: None`
2. Owner calls `authorize_extension<Auth>()` to set extension
3. Owner calls `freeze_extension_config()` -- adds DF marker, emits `ExtensionConfigFrozenEvent`
4. Extension is now immutable -- authorize/revoke abort
5. On unanchor (destroy), `remove_frozen_marker_if_present()` cleans up the DF

### dApp Integration

**Check frozen status:**
- Query assembly dynamic fields for `ExtensionFrozenKey` type, or
- Call `is_extension_frozen()` via devInspect, or
- Index `ExtensionConfigFrozenEvent` (freeze is irreversible, so once seen it stays frozen)

**TX building:** PTB calls `{package}::gate::freeze_extension_config` (or turret/storage_unit variant) passing assembly object + OwnerCap borrowed from Character.

**UX:** Permanent action -- should show strong confirmation dialog. After freeze, disable "Change Extension" / "Remove Extension" UI.
...
Gate extension that restricts jump access to a single tribe.

**Setup flow:**
1. Deploy the extension package -- creates shared `ExtensionConfig` + transfers `AdminCap` to deployer
2. Call `authorize_extension<XAuth>(gate, owner_cap)` on both source and destination gates
3. Call `set_tribe_config(extension_config, admin_cap, tribe_id)` to set the allowed tribe

**Permit issuance:**
1. Character calls the extension's `issue_jump_permit()`
2. Extension reads `TribeConfig` from shared `ExtensionConfig` dynamic field
3. Asserts `character.tribe() == tribe_cfg.tribe` (aborts `ENotStarterTribe` otherwise)
4. Issues permit with 5-day expiry via `gate::issue_jump_permit_with_id<XAuth>(...)`
5. Permit is transferred to character's address

**Relevance for Periscope:** This is the pattern for standings-gated gates. Could build a variant that checks against a standings list stored in the extension config rather than a single tribe ID.

### `extension_examples::corpse_gate_bounty`

Gate + StorageUnit combo extension: character must deposit a specific item type (corpse) into a storage unit before receiving a jump permit. Demonstrates cross-assembly extension patterns.
...
1. **hp_ratio, shield_ratio, armor_ratio** — Public getters added in v0.0.19 (PR #135). Extensions can now read `hp_ratio()`, `shield_ratio()`, `armor_ratio()` on `TargetCandidate` for custom turret targeting logic.
